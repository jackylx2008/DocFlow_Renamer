...
1. 人工将飞书审批结束后生成的 PDF（通过或拒绝）放入 `_input`。
...
6. 从 PDF 顶部“审批状态”字段提取 `approval.result`：通过为 `approved`，
   被拒绝为 `rejected`，并更新案卷状态。
7. 根据 JSON 更新汇总 HTML。
...
所有入口统一使用项目根目录的 `logging_config.py`，同时输出控制台日志和
滚动文件日志。日志目录由 `config.yaml` 的 `app.log_dir` 配置，文件名根据
实际入口生成，例如 `run_archive.log`、`migrate_archive.log` 和
`serve_archive_review.log`。单个日志文件最大 10 MB，保留 5 份备份。

`setup_logger()` 的文件参数为 `log_file`，调用方应传入包含文件名的完整路径；
业务代码不再传递旧版的 `log_dir` 或 `entry_name` 参数，也不在业务模块中
自行添加日志 handler。
...
两项使用分号分隔并换行，缺少的材料直接列在该列中。“审批结果”列根据
正式 JSON 的 `approval.result` 显示“通过”或“被拒绝”；历史审批 PDF
没有该字段时按通过兼容显示。
没有对应审批 PDF 的记录会在“审批PDF”列显示“选择审批单”和“保存 PDF”；
人工选择的 PDF 经格式、数据版本和案卷状态校验后直接归档，并刷新正式 JSON
及汇总页面。单个 PDF 最大 50 MB，此功能须通过页面启动器使用。
...
审批完成的条目使用约 54px 的紧凑行高，约为原常见行高的三分之一；内容
超过单元格可见高度时，仅在该单元格内部显示细滚动条，不再拉高整行。
打印时紧凑行会自动展开，保证完整内容可见。
每条记录末列提供人工审批结果下拉框，可在“通过”和“被拒绝”之间选择并
保存；页面以正式 JSON 中已有的 `approval.result` 作为默认值。该列同时
提供“删除”按钮，确认后从正式数据移除记录，并将整个案卷目录移动到
`_trash/cases` 以便恢复。保存和删除只在通过 `.cmd` 或 `.command` 启动的
localhost 页面中可用，操作时校验数据版本和案卷 ID，完成后同步刷新正式
JSON、汇总 HTML 与审批审核页面。

页面支持页签切换、当前页签搜索、固定表头和打印。HTML 本身仍不是事实
数据源，所有人工操作均由本地服务校验后写回正式 JSON。
